<template>
  <div>
    <div class="margintop" v-show="step == 0">
      <div class="fundtitle">
        {{fundCode == '000662'?'现金宝':fundName}}<span>({{fundCode}})</span>
      </div>
      <group label-width="4.5em" label-margin-right="2em" label-align="left" style="margin-top: 10px;">
        ...
        <x-input title="购买金额" :placeholder="minMoney" v-model="buyMoney" text-align="right"
                 @on-blur="getFundSingleBuyCharge" type='number'></x-input>
  ...
  export default {
    ...
    methods: {
      ...
      getParams () {
        ...
        this.fundType = this.$route.query.fundType
        this.perMax22 = this.$route.query.perMax22
        this.perMin22 = this.$route.query.perMin22
        this.minMoney = this.$route.query.perMin22 + '元起购'
        this.perMax24 = this.$route.query.perMax24
        this.perMin24 = this.$route.query.perMin24
        this.perMax20 = this.$route.query.perMax20
        this.perMin20 = this.$route.query.perMin20
      ...
      getPublicKey () {
        ...
        if (Number(this.buyMoney) < Number(this.perMin22)) {
      ...
      buyFund (keyPass) {
        this.$http.post(mianUrlt9 + 'fund/singleBuy', {
          applicationAmount: this.buyMoney,
          ...
          if (res.data.retCode === '0') {
            ...
            this.ables = false
            if (this.fundCode === '000662') {
              this.description = '您购买的现金宝' + this.buyMoney + '元已经成功！'
            } else {
              this.description = '您购买的' + this.fundName + '基金' + this.buyMoney + '元已经成功！'
            }
            this.step = 1

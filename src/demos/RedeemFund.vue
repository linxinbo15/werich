<template>
  <div>
    <div class="margintop" v-show="step == 0">
      <div class="fundtitle">
        <div>基金名称</div>
        <div class="fundtitle_right">{{fundName.length>13 ? fundName.substring(0,13)+'...':fundName}}<span>({{fundCode}})</span></div>
      </div>
      <div class="fundtitle">
        <div>可用份额</div>
      ...
      <group label-width="4.5em" label-margin-right="2em" label-align="right" style="margin-top: 10px;">
        ...
        <x-input title="赎回份额" :placeholder="availableVolText" v-model="redeemMoney" text-align="right">
  ...
  export default {
    ...
    methods: {
      ...
      getParams () {
        ...
        this.availableVolText = '最少赎回' + this.$route.query.redeemMinVol + '份'
        ...
        if(this.$route.query.isAutoInvestSell){
           this.isAutoInvestSell = this.$route.query.isAutoInvestSell
        }
        // 将数据放在当前组件的数据内
      },
      getuniqueid () {
      ...
      redeemFund (keyPass) {
        ...
        this.$http.post(mianUrlt9 + 'fund/singleSell', obj, {emulateJSON: true}).then((res) => {
          if (res.data.retCode === '0') {
            this.$vux.loading.hide()
            this.ables = false
            this.step = 1
            this.description = '您赎回的' + res.data.data.fundName + '基金' + res.data.data.applicationVol + '份已经成功！'

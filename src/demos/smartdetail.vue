<template>
  <div class='detail'>
    <div class="c_box">
      选定大金融、大消费、技术创新、高端制造四个板块为策略基准，选取投资于该方向的四只优质基金，并根据多因子模型进行定期权重调整。
    </div>
    <div class="c_des">
      <!--<div class="des_top">
    ...
    <div class="con2">
      ...
      <!--<ul class="time">
        <li class="dipper-date" v-for="(item,index) in tabs" :class="{active:index == number}"
            @click='getchartdata("smart1", item.suo, index)'>{{item.name}}
        </li>
      </ul>-->
      <button-tab>
        <button-tab-item selected @on-item-click="getchartdata('smart1', 6 , 0)">成立以来</button-tab-item>
        <button-tab-item @on-item-click="getchartdata('smart1', 1, 1)">近三月</button-tab-item>
        <button-tab-item @on-item-click="getchartdata('smart1', 2, 2)">近半年</button-tab-item>
        <button-tab-item @on-item-click="getchartdata('smart1', 3, 3)">近一年</button-tab-item>
      </button-tab>
    ...
    <group v-show="currentIndex === 0">
      ...
      <div class="test_grade1">
        <ul class="test_grade2 prop">
          <li @click="goFundDetail('易方达中小盘','110011.OF','混合型')">
            <h2>易方达中小盘(<span>110011</span>)</h2><span>25%</span>
            <div class="dtline" style="width:25%;"></div>
          </li>
          <li @click="goFundDetail('前海开源沪港深蓝筹','001837.OF','混合型')">
            <h2>前海开源沪港深蓝筹(<span>001837</span>)</h2><span>50%</span>
            <div class="dtline1"></div>
          </li>
          <li @click="goFundDetail('易方达新兴成长','000404.OF','混合型')">
            <h2>易方达新兴成长(<span>000404</span>)</h2><span>25%</span>
            <div class="dtline" style="width:25%;"></div>
          </li>
          <li @click="goFundDetail('前海开源高端装备制造','001060.OF','混合型')">
            <h2>前海开源高端装备制造(<span>001060</span>)</h2><span>0%</span>
            <div></div>
          </li>
        </ul>
      </div>
    ...
    methods: {
      ...
      queryCJGroupTranSferRecord () {
        ...
        this.$http.post(mianUrlt9 + 'fund/queryTacticsProportionInfo', obj, {emulateJSON: true}).then(({data}) => {
          if (data.retCode === '0') {
             // console.log(data.data.proportionInfo[1][0])

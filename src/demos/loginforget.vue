<template>
  <div>
    <group>
      <x-input title="手机号码:" :max="13" is-type="china-mobile" placeholder="请输入手机号" v-model="phone"></x-input>
      ...
      <x-input title="设置密码:" type="password" placeholder="请输入不少于6位密码" v-model='tpwd1'></x-input>
      <x-input title="确认密码:" type="password" placeholder="请确认密码" v-model='tpwd2'></x-input>
    </group>
    <box gap="10px 10px">
  ...
  export default {
    ...
    methods: {
      ...
      getvalid () { // 调短信接口
        ...
        this.$http.post(mianUrlt9 + 'user/sendFindLoginPasswordCode', {
          mobile: this.phone,
      ...
      resettpwd (keyTpwd1) {
        this.$http.post(mianUrlt9 + 'user/resetLoginPassword', {
          password: keyTpwd1,
          smsCode: this.valid,
          phone: this.phone
        }, {emulateJSON: true}).then((res) => {

  ...
  <div>
    <div class="margintop" v-show="step == 0">
      ...
      <group label-width="4.5em" label-margin-right="2em" label-align="right" style="margin-top: 10px;">
        ...
        <x-input title="定投金额" :placeholder="minMoney" v-model="buyMoney" text-align="right"></x-input>
        <!--@on-blur="getFundSingleBuyCharge"手续费查询接口定投没有-->
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      /* getFundSingleBuyCharge () {
        let money_num = this.buyMoney
        if (this.buyMoney == '') {
          money_num = 0
        }
        const obj ={
          fundCode: this.fundCode,
          purchasePrice: money_num
        }
        this.$http.post(mianUrlt9 + 'fund/getFundSingleBuyCharge', obj, { emulateJSON: true }).then((res) => {
      ...
      getParams () {
        ...
        this.maxInvestAmount = this.$route.query.maxInvestAmount
        this.minInvestAmount = this.$route.query.minInvestAmount
        this.minMoney = '起购金额' + this.minInvestAmount + '元'
        // 将数据放在当前组件的数据内
        for (let i = 1; i <= 28; i++) {
          this.listc[0].push(i + '号')
        }
      ...
      getValue (ref) {
        ...
        this.moneyAccount = this.$refs[ref].getFullValue()[0].id
      ...
      getPublicKey () {
        if (this.buyMoney === '' || this.buyMoney == null) {
          this.toastText = '请输入定投金额'
          this.showPosition('middle')
          return
        }
        if (Number(this.buyMoney) < Number(this.minInvestAmount)) {
          this.toastText = '定投金额低于最低额度'
          this.showPosition('middle')
          return
        }
        ...
        this.ables = true
        this.$vux.loading.show({text: '定投协议生成中'})
      ...
      buyFund (keyPass) {
        let investDay = this.date_invest[0].replace(/[^0-9]/ig, '')
        this.$http.post(mianUrlt9 + 'fund/generalInvestOpen', {
          fundCode: this.fundCode.substring(0, 6),
          investFrequency: this.frequency,
          investDay: parseInt(investDay),
          cycleMoney: this.buyMoney,
          ...
          if (res.data.retCode === '0') {
            ...
            this.ables = false
            this.step = 1
            this.description = '您定投的' + this.fundName + '基金，每期' + this.buyMoney + '元已经开通！'
